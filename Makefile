...
no_arguments:
	...
	@echo "  - test-labplus-xv6ls-check: Run xv6 shell ls and check directory output"
	...
	@echo "  - test-labplus-xv6fswrite-check: Run xv6 shell file create/write/read smoke"
	@echo "  - test-labplus-xv6user-check: Run xv6 shell/ls/cat/grep/wc/fswrite smoke checks"
...
XV6_LS_INPUT_HEX ?= 6c730a
XV6_LS_EXPECT ?= README
XV6_LS_LOG ?= build/xv6/ls.log
XV6_CAT_INPUT_HEX ?= 63617420524541444d450a
XV6_CAT_EXPECT ?= xv6 is a re-implementation
XV6_CAT_LOG ?= build/xv6/cat.log
XV6_GREP_INPUT_HEX ?= 677265702078763620524541444d450a
XV6_GREP_EXPECT ?= xv6 is
XV6_GREP_LOG ?= build/xv6/grep.log
XV6_WC_INPUT_HEX ?= 776320524541444d450a
XV6_WC_EXPECT ?= 2441 README
XV6_WC_LOG ?= build/xv6/wc.log
XV6_FSWRITE_INPUT_HEX ?= 6563686f20616263203e20746d700a776320746d700a
XV6_FSWRITE_EXPECT ?= 4 tmp
XV6_FSWRITE_LOG ?= build/xv6/fswrite.log
XV6_SRC ?=
XV6_KERNEL_ELF ?=
XV6_FS_IMG ?=
XV6_OBJCOPY ?=
...
test-labplus-sstc:
	rm -rf build/sstc
	verilator --binary --timing --top-module sstc_tb \
	  +define+VERILATOR=1 -I$(NOOP_HOME)/vsrc \
...
test-labplus-mcountinhibit:
	rm -rf build/csr-mcountinhibit
	verilator --binary --timing --top-module csr_mcountinhibit_tb \
	  +define+VERILATOR=1 -I$(NOOP_HOME)/vsrc \
	  -Mdir build/csr-mcountinhibit -o csr_mcountinhibit_tb \
	  vsrc/test/difftest_stubs.sv vsrc/test/csr_mcountinhibit_tb.sv
	./build/csr-mcountinhibit/csr_mcountinhibit_tb
...
test-labplus-xv6ls-check:
	python3 tools/check_xv6_boot.py \
	  --kernel "$(XV6_KERNEL)" \
	  --fs "$(XV6_FS)" \
	  --max-cycles "$(XV6_SHELL_MAX_CYCLES)" \
	  --vopt "-- +uart_input_hex=$(XV6_LS_INPUT_HEX) +uart_input_gap=$(XV6_SHELL_INPUT_GAP) $(VOPT)" \
	  --expect "$(XV6_LS_EXPECT)" \
	  --log "$(XV6_LS_LOG)"

test-labplus-xv6cat-check:
	python3 tools/check_xv6_boot.py \
	  --kernel "$(XV6_KERNEL)" \
	  --fs "$(XV6_FS)" \
	  --max-cycles "$(XV6_SHELL_MAX_CYCLES)" \
	  --vopt "-- +uart_input_hex=$(XV6_CAT_INPUT_HEX) +uart_input_gap=$(XV6_SHELL_INPUT_GAP) $(VOPT)" \
	  --expect "$(XV6_CAT_EXPECT)" \
	  --log "$(XV6_CAT_LOG)"

test-labplus-xv6grep-check:
	python3 tools/check_xv6_boot.py \
	  --kernel "$(XV6_KERNEL)" \
	  --fs "$(XV6_FS)" \
	  --max-cycles "$(XV6_SHELL_MAX_CYCLES)" \
	  --vopt "-- +uart_input_hex=$(XV6_GREP_INPUT_HEX) +uart_input_gap=$(XV6_SHELL_INPUT_GAP) $(VOPT)" \
	  --expect "$(XV6_GREP_EXPECT)" \
	  --log "$(XV6_GREP_LOG)"

test-labplus-xv6wc-check:
	python3 tools/check_xv6_boot.py \
	  --kernel "$(XV6_KERNEL)" \
	  --fs "$(XV6_FS)" \
	  --max-cycles "$(XV6_SHELL_MAX_CYCLES)" \
	  --vopt "-- +uart_input_hex=$(XV6_WC_INPUT_HEX) +uart_input_gap=$(XV6_SHELL_INPUT_GAP) $(VOPT)" \
	  --expect "$(XV6_WC_EXPECT)" \
	  --log "$(XV6_WC_LOG)"

test-labplus-xv6fswrite-check:
	python3 tools/check_xv6_boot.py \
	  --kernel "$(XV6_KERNEL)" \
	  --fs "$(XV6_FS)" \
	  --max-cycles "$(XV6_SHELL_MAX_CYCLES)" \
	  --vopt "-- +uart_input_hex=$(XV6_FSWRITE_INPUT_HEX) +uart_input_gap=$(XV6_SHELL_INPUT_GAP) $(VOPT)" \
	  --expect "$(XV6_FSWRITE_EXPECT)" \
	  --log "$(XV6_FSWRITE_LOG)"
...
test-labplus-xv6user-check:
	...
	$(MAKE) test-labplus-xv6fswrite-check
