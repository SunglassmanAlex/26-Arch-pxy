...
\begin{itemize}
  ...
  \item 新增仿真侧 16550 UART MMIO 模型：在 \texttt{0x10000000} 兼容 xv6/QEMU UART 初始化和 TX 输出路径，并连接到 difftest UART 输出端口。
...
\begin{itemize}
  \item offset 0：\texttt{RBR/THR/DLL}。\texttt{LCR.DLAB=1} 时写入/读出 DLL；否则写 THR 产生一个 \texttt{uart\_out\_valid/ch} 输出脉冲。
  \item offset 1：\texttt{IER/DLM}。\texttt{LCR.DLAB=1} 时访问 DLM；否则访问 IER。
  \item offset 2：\texttt{IIR/FCR}。读 IIR 固定返回 \texttt{0x01}，表示当前无 UART 中断 pending；写 FCR 被接受但不维护 FIFO 状态。
  \item offset 3/4/7：\texttt{LCR/MCR/SCR} 可读写。
  \item offset 5：\texttt{LSR} 固定返回 \texttt{0x60}，表示 THR empty 和 transmitter empty，满足 xv6 \texttt{uartputc} 的轮询条件。
\end{itemize}

\texttt{SimTop} 不再把 difftest UART 端口固定为 0，而是将 \texttt{SimMemoryWithVirtio} 的 TX 输出连接到 \texttt{io\_uart\_out\_valid/io\_uart\_out\_ch}。RX 侧目前保留接口但不主动提供字符，因此 \texttt{LSR.RX\_READY=0}；后续如果要做交互式输入，再把 host 输入缓存和 PLIC source 10 接进来即可。
...
\begin{itemize}
  ...
  \item \texttt{vsrc/util/SimMemoryWithVirtio.sv}：新增仿真内存包装器，转发普通 RAM/CLINT 请求并处理 \texttt{0x10001000} virtio/simple-block MMIO；新增 PLIC MMIO 模型，拦截 \texttt{0x0c000000} 到 \texttt{0x0fffffff}，支持 source priority、pending、M/S enable、threshold、claim/complete；新增 \texttt{0x10000000} 16550 UART MMIO 模型，支持 LCR/DLAB、DLL/DLM、IER、MCR、SCR、LSR 和 THR TX 输出。
  \item \texttt{vsrc/SimTop.sv}：将仿真 UART TX 输出接到 difftest 顶层 \texttt{io\_uart\_out\_valid/io\_uart\_out\_ch}。
  ...
  \item \texttt{vsrc/test/uart\_mmio\_tb.sv}：新增 UART MMIO 定向测试，覆盖 xv6 常用初始化路径和 TX 输出。
...
该测试直接实例化 \texttt{SimMemoryWithVirtio}，用 8-bit CBus 访问模拟 xv6 的 UART 初始化流程：先设置 \texttt{LCR.DLAB} 写 DLL/DLM，再恢复 8N1、写 IER/MCR/SCR，最后写 THR 并检查 \texttt{uart\_out\_valid/ch} 输出字符 \texttt{A}。
...
\begin{itemize}
  ...
  \item 将 UART RX、UART RX interrupt、完整 virtio queue 中断等更多真实设备事件接入 PLIC source。
...
本次 Lab+ 新增完成了 atomic extension、8-entry PMP/privfull 支持、\texttt{EBREAK} 断点异常、\texttt{FENCE/FENCE.I} 兼容，并加入顺序取指提前、CBus fast path 和 8B 指令行缓冲等前端性能优化。继续推进 xv6 主 Track 时，已完成 S-mode、\texttt{SRET}、trap delegation、S 态中断 pending 委托转换、MMU page fault/PTE 基础权限检查、\texttt{SUM/MXR} 权限补充、PTE A/D 位硬件更新、简化 virtio/disk MMIO、仿真侧 PLIC MMIO 模型、最小 16550 UART TX 模型，以及独立 MMU page fault、S interrupt、PLIC、UART 和 simple-block 定向测试。AMO 实现利用现有单发访存结构，将 AMO 指令拆成不可被其他指令插入的读-改-写序列，并为 \texttt{LR.W/SC.W} 添加 reservation 状态。性能优化将 Lab1 extra 周期数从 185976 降到 93022，将 Lab4 周期数从 208529 降到 110574，atomicity 从 372 降到 197。最终 atomic、Lab+ privileged sys-test、MMU page fault directed tests、S-mode interrupt directed test、PLIC MMIO directed test、UART MMIO directed test、simple virtio block MMIO test、Lab1 extra、Lab4、Lab5、Lab6 均完成回归。
