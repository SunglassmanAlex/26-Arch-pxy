...
\begin{itemize}
  ...
  \item 新增仿真侧 virtio/disk MMIO：在 \texttt{0x10001000} 暴露 virtio-mmio 识别寄存器，提供基础 feature negotiation，提供同步 512B sector 读写扩展，支持单 queue 的 descriptor/avail/used ring 与 indirect descriptor 子集，并支持 \texttt{+simple\_blk\_image=...} 从二进制镜像初始化 disk。
...
\begin{itemize}
  ...
  \item \texttt{vsrc/test/simple\_virtio\_block\_tb.sv}、\texttt{vsrc/test/ram\_dpi\_stubs.cpp}：新增 simple-block MMIO 定向测试和 RAM DPI stub，验证镜像初始化、未知命令/越界 sector 错误状态、feature negotiation、virtqueue read/write、indirect descriptor read、512B sector write/read。
