...
\begin{itemize}
  \item \texttt{DeviceFeaturesSel/DriverFeaturesSel} 按 32-bit bank 选择 feature，当前广告 \texttt{VIRTQ\_DESC\_F\_INDIRECT}、\texttt{VIRTIO\_RING\_F\_EVENT\_IDX} 和 \texttt{VIRTIO\_F\_VERSION\_1}；写 \texttt{Status.FEATURES\_OK} 时会检查 driver 是否写入 unsupported feature，若有则清除 \texttt{FEATURES\_OK}。
...
\begin{itemize}
  ...
  \item \texttt{vsrc/test/simple\_virtio\_block\_tb.sv}、\texttt{vsrc/test/ram\_dpi\_stubs.cpp}：新增 simple-block MMIO 定向测试和 RAM DPI stub，验证镜像初始化、virtio-blk config 字段、\texttt{ConfigGeneration}、未知命令/越界 sector 错误状态、feature negotiation、virtqueue read/write、indirect descriptor read、event idx 抑制/触发中断、\texttt{Status=0} reset、512B sector write/read。
...
\begin{itemize}
  \item 更完整的 virtio block feature 组合、多 queue、packed queue 和动态配置变更通知。
