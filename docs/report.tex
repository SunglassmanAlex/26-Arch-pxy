...
\begin{itemize}
  ...
  \item 新增仿真侧 16550 UART MMIO 模型：在 \texttt{0x10000000} 兼容 xv6/QEMU UART 初始化、TX 输出、16B RX FIFO/RBR 读取、FCR trigger/clear 和 LSR overrun，并将 UART RX interrupt 接到 PLIC source 10。
...
\begin{itemize}
  ...
  \item offset 2：\texttt{IIR/FCR}。无 UART 中断 pending 时读 IIR 返回 \texttt{0x01}；当 \texttt{IER.RX=1} 且 RX FIFO 深度达到 FCR trigger 时返回 \texttt{0x04}，表示 received data available；当 \texttt{IER.RLS=1} 且发生 overrun 时返回 \texttt{0x06}，表示 receiver line status 优先级更高。写 FCR 会保存 bit7:6 的 RX trigger，并支持 bit1 清空 RX FIFO。
...
\begin{itemize}
  ...
  \item \texttt{vsrc/util/SimMemoryWithVirtio.sv}：新增仿真内存包装器，转发普通 RAM/CLINT 请求并处理 \texttt{0x10001000} virtio/simple-block MMIO；新增 PLIC MMIO 模型，拦截 \texttt{0x0c000000} 到 \texttt{0x0fffffff}，支持 source priority、pending、M/S enable、threshold、claim/complete；新增 \texttt{0x10000000} 16550 UART MMIO 模型，支持 LCR/DLAB、DLL/DLM、IER、FCR trigger/clear、MCR、SCR、LSR overrun、THR TX 输出、16B RBR RX FIFO 和 RX interrupt 到 PLIC source 10。
  ...
  \item \texttt{vsrc/test/uart\_mmio\_tb.sv}：新增 UART MMIO 定向测试，覆盖 xv6 常用初始化路径、TX 输出、RX ready、RBR FIFO 顺序读取、FCR trigger/clear、overrun 和 PLIC source 10 claim。
...
该测试直接实例化 \texttt{SimMemoryWithVirtio}，用 8-bit CBus 访问模拟 xv6 的 UART 初始化流程：先设置 \texttt{LCR.DLAB} 写 DLL/DLM，再恢复 8N1、写 IER/MCR/SCR，写 THR 并检查 \texttt{uart\_out\_valid/ch} 输出字符 \texttt{A}。随后配置 PLIC source 10，注入 RX 字符 \texttt{B}，验证 \texttt{LSR.RX\_READY}、\texttt{IIR=0x04}、PLIC pending/exint、读 RBR 清 RX-ready，以及 PLIC claim/complete。最后覆盖 16B FIFO 顺序读出、FCR trigger=4 时前三个字符不触发中断/第四个字符触发、FCR bit1 清 RX FIFO、满 FIFO 反压输入、强制 overrun 后 IIR line-status 优先级、LSR sticky overrun 读后清除。
...
\begin{itemize}
  ...
  \item 更完整的 16550 THRE interrupt、FIFO timeout interrupt、parity/framing/break 等线状态错误。
