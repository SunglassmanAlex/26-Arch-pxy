...
\begin{itemize}
  ...
  \item 新增仿真侧 virtio/disk MMIO：在 \texttt{0x10001000} 暴露 virtio-mmio 识别寄存器和 virtio-blk config 字段，提供基础 feature negotiation，提供同步 512B sector 读写扩展，支持单 queue 的 descriptor/avail/used ring 与 indirect descriptor 子集，并支持 \texttt{+simple\_blk\_image=...} 从二进制镜像初始化 disk。
...
\begin{itemize}
  \item \texttt{DeviceFeaturesSel/DriverFeaturesSel} 按 32-bit bank 选择 feature，当前广告 \texttt{VIRTQ\_DESC\_F\_INDIRECT} 和 \texttt{VIRTIO\_F\_VERSION\_1}；写 \texttt{Status.FEATURES\_OK} 时会检查 driver 是否写入 unsupported feature，若有则清除 \texttt{FEATURES\_OK}。
  ...
  \item 完成 queue 请求后置位 \texttt{InterruptStatus[0]}，并复用 PLIC source 1 pending；软件可通过 \texttt{InterruptACK} 清除 virtio 中断状态。
\end{itemize}
...
\begin{itemize}
  ...
  \item \texttt{vsrc/test/simple\_virtio\_block\_tb.sv}、\texttt{vsrc/test/ram\_dpi\_stubs.cpp}：新增 simple-block MMIO 定向测试和 RAM DPI stub，验证镜像初始化、virtio-blk config 字段、未知命令/越界 sector 错误状态、feature negotiation、virtqueue read/write、indirect descriptor read、512B sector write/read。
...
\begin{itemize}
  \item 更完整的 virtio block feature 组合、event idx、多 queue 和 reset/config generation。
