...
\begin{itemize}
  ...
  \item 新增仿真侧 16550 UART MMIO 模型：在 \texttt{0x10000000} 兼容 xv6/QEMU UART 初始化、TX 输出、RX ready/RBR 读取，并将 UART RX interrupt 接到 PLIC source 10。
...
\begin{itemize}
  \item offset 0：\texttt{RBR/THR/DLL}。\texttt{LCR.DLAB=1} 时写入/读出 DLL；否则写 THR 产生一个 \texttt{uart\_out\_valid/ch} 输出脉冲，读 RBR 返回 RX 缓冲字符并清除 \texttt{LSR.RX\_READY}。
  ...
  \item offset 5：\texttt{LSR} 固定保持 \texttt{THR empty/transmitter empty=1}，并根据 RX 缓冲状态动态维护 bit0 \texttt{RX\_READY}，满足 xv6 \texttt{uartputc} 和 \texttt{uartgetc} 的轮询条件。
\end{itemize}

\texttt{SimTop} 不再把 difftest UART 端口固定为 0，而是将 \texttt{SimMemoryWithVirtio} 的 TX 输出连接到 \texttt{io\_uart\_out\_valid/io\_uart\_out\_ch}。RX 侧将 \texttt{io\_uart\_in\_ch=8'hff} 视为无字符，其它值会进入一字节 RX 缓冲；若 \texttt{IER.RX=1}，同时置位 PLIC source 10 pending，软件可以通过 PLIC claim/complete 接收 UART RX 外部中断。
...
\begin{itemize}
  ...
  \item \texttt{vsrc/util/SimMemoryWithVirtio.sv}：新增仿真内存包装器，转发普通 RAM/CLINT 请求并处理 \texttt{0x10001000} virtio/simple-block MMIO；新增 PLIC MMIO 模型，拦截 \texttt{0x0c000000} 到 \texttt{0x0fffffff}，支持 source priority、pending、M/S enable、threshold、claim/complete；新增 \texttt{0x10000000} 16550 UART MMIO 模型，支持 LCR/DLAB、DLL/DLM、IER、MCR、SCR、LSR、THR TX 输出、RBR RX 读取和 RX interrupt 到 PLIC source 10。
  ...
  \item \texttt{vsrc/test/uart\_mmio\_tb.sv}：新增 UART MMIO 定向测试，覆盖 xv6 常用初始化路径、TX 输出、RX ready/RBR 清除和 PLIC source 10 claim。
...
该测试直接实例化 \texttt{SimMemoryWithVirtio}，用 8-bit CBus 访问模拟 xv6 的 UART 初始化流程：先设置 \texttt{LCR.DLAB} 写 DLL/DLM，再恢复 8N1、写 IER/MCR/SCR，写 THR 并检查 \texttt{uart\_out\_valid/ch} 输出字符 \texttt{A}。随后配置 PLIC source 10，注入 RX 字符 \texttt{B}，验证 \texttt{LSR.RX\_READY}、\texttt{IIR=0x04}、PLIC pending/exint、读 RBR 清 RX-ready，以及 PLIC claim/complete。
...
\begin{itemize}
  ...
  \item 将当前单字节 UART RX 缓冲扩展为 FIFO，并补 host 侧交互输入。
...
本次 Lab+ 新增完成了 atomic extension、8-entry PMP/privfull 支持、\texttt{EBREAK} 断点异常、\texttt{FENCE/FENCE.I} 兼容，并加入顺序取指提前、CBus fast path 和 8B 指令行缓冲等前端性能优化。继续推进 xv6 主 Track 时，已完成 S-mode、\texttt{SRET}、trap delegation、S 态中断 pending 委托转换、MMU page fault/PTE 基础权限检查、\texttt{SUM/MXR} 权限补充、PTE A/D 位硬件更新、简化 virtio/disk MMIO、仿真侧 PLIC MMIO 模型、最小 16550 UART TX/RX interrupt 模型，以及独立 MMU page fault、S interrupt、PLIC、UART 和 simple-block 定向测试。AMO 实现利用现有单发访存结构，将 AMO 指令拆成不可被其他指令插入的读-改-写序列，并为 \texttt{LR.W/SC.W} 添加 reservation 状态。性能优化将 Lab1 extra 周期数从 185976 降到 93022，将 Lab4 周期数从 208529 降到 110574，atomicity 从 372 降到 197。最终 atomic、Lab+ privileged sys-test、MMU page fault directed tests、S-mode interrupt directed test、PLIC MMIO directed test、UART MMIO directed test、simple virtio block MMIO test、Lab1 extra、Lab4、Lab5、Lab6 均完成回归。
