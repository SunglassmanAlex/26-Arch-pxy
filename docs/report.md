...
- 新增 Lab+ atomic extension：实现 AMO W/D 系列指令以及 `LR.W/SC.W/LR.D/SC.D`，并接入 difftest atomic event。
...
- 新增 `mstatus.TSR/TW/TVM` 限制检查：S 态在对应位打开时执行 `SRET/WFI/SFENCE.VMA` 或访问 `satp` 会按 illegal instruction trap 处理。
...
- 新增 environment/config CSR 探测兼容：支持 `mconfigptr` 只读返回 0，支持 `menvcfg.STCE` WARL 写入和 `senvcfg` WARL zero，避免标准启动代码探测这些 CSR 时直接 illegal，并为 Sstc 定时器路径提供开关。
...
- 新增 xv6 `ls` 目录读取检查入口：`make test-labplus-xv6ls-check` 在 shell 中注入 `ls`，检查文件系统目录输出中是否包含 `README`，进一步覆盖用户程序执行、virtio 文件系统镜像读取和目录项解析路径。
...
- 新增 xv6 文件写入检查入口：`make test-labplus-xv6fswrite-check` 在 shell 中执行 `echo abc > tmp` 后 `wc tmp`，检查 `tmp` 的字节统计输出，覆盖文件创建、写入、读回和 virtio disk 写路径。
- 新增 xv6 userland smoke 汇总入口：`make test-labplus-xv6user-check` 顺序运行 shell、`ls`、`cat`、`grep`、`wc` 和文件写入 smoke，便于后续一键回归。
...
- 新增 S 态软件中断定向测试，覆盖 delegated SSIP 从硬件 `swint` 进入 S trap 的路径。
- 新增 S 态外部中断定向测试，覆盖 delegated SEIP 从 `exint` 进入 S trap 的路径。
- 新增 M timer from S-mode 定向测试，覆盖 xv6 stock timervec 常用的 S 态运行时 MTIP 回到 M trap 路径。
- 新增 timervec SSIP handoff 定向测试，覆盖 M timer handler 写 `sip.SSIP` 后返回 S 态并进入 S software interrupt 的 xv6 常见交接路径。
...
- 新增 mstatus TSR/TW/TVM 定向测试，覆盖 S 态被限制的 `satp`、`SFENCE.VMA`、`WFI` 和 `SRET` 都进入 illegal instruction trap。
...
- 新增 CSR envcfg 定向测试，覆盖 `mconfigptr/menvcfg/senvcfg` M 态探测、`menvcfg.STCE` WARL 保留、`senvcfg` WARL zero，以及 S 态访问 `senvcfg`。
...
`LR.W/D` 要求 `rs2=0`。所有 AMO 指令按照 store/AMO 地址对齐规则检查地址低位，W 要求 4B 对齐，D 要求 8B 对齐，不对齐时产生 store/AMO address misaligned 异常。
...
`LR.W/D` 只执行读请求，并按访问宽度记录 4B 或 8B 对齐后的 reservation 地址。`SC.W/D` 在 reservation 命中时发出写请求并返回 0；未命中时不访问内存，直接返回 1，并清除 reservation。当前官方 `atomicity.bin` 测试覆盖的是成功的 `SC.W` 路径，补充的 directed test 覆盖了 `SC.D` 成功与失败。
...
- `atomicData` 使用未移位的 `rs2`，`atomicOut` 使用旧值，`atomicMask` 根据地址低三位和 AMO 宽度生成 `0x0f/0xf0/0xff`。
...
官方 Lab+ 主 Track 是尝试运行 xv6。本次先补 xv6 boot 的 CPU 前置能力和 QEMU virt 平台外设路径，随后使用 no-RVC xv6 smoke 镜像实际跑到 `init: starting sh`。新增内容如下：
...
这一部分把 xv6 所需的 Supervisor trap 基础路径补上了，并通过现有 Lab5、Lab6 与 Lab+ privfull 回归确认没有破坏原 U/M 行为。后续章节继续补充 CLINT、PLIC、16550 UART、virtio block、Sstc `stimecmp` 和 boot target；当前提交还包含 `ready-to-run/xv6/kernel.bin` 与 `fs.img`，默认 boot check 已经可以直接复测到 shell banner。需要说明的是，该镜像是为了当前无 RVC CPU 和 RTL 仿真速度准备的 smoke 版本，不等同于 stock xv6 完整回归。
...
- 支持 queue 0 到 queue 3，`QueueNumMax=8`，4 条 queue 都有独立的 `QueueNum/QueueReady/QueueDescLow/High/QueueDriverLow/High/QueueDeviceLow/High/last_avail_idx` 状态；`QueueNotify` 按写入的 queue id 选择对应 queue，而不依赖当前 `QueueSel`，越界 queue 的 `QueueNumMax` 返回 0。
- 支持 `QueueReset`：向当前 `QueueSel` 对应的 queue 写非 0 会清空该 queue 的 num/ready/desc/driver/device/last_avail_idx，读该寄存器返回 0；其它 queue 不受影响。
- `QueueNotify` 时从对应 queue 的 `virt_last_avail_idx` drain 到当前 `avail.idx`，逐个读取 pending head descriptor，并按 virtio block descriptor 链处理：普通读写使用 request header、512B 对齐 data buffer、1B status 三段链；flush 使用 request header、1B writable status 两段链。
- 当 head descriptor 带 `VIRTQ_DESC_F_INDIRECT` 时，将其 `addr/len` 解释为 indirect descriptor table，并从 table 内继续解析 descriptor 链。
- 支持 `VIRTIO_BLK_T_IN=0` 从 disk 读入 driver writable buffer，支持 `VIRTIO_BLK_T_OUT=1` 从 driver buffer 写回 disk，支持 `VIRTIO_BLK_T_FLUSH=4` 返回 status-only 完成，支持 `VIRTIO_BLK_T_DISCARD=11` 读取 16B range 并 no-op 成功，支持 `VIRTIO_BLK_T_WRITE_ZEROES=13` 读取 16B range 并清零对应 sector；普通 IN/OUT 的 data descriptor 长度可以是 512B 的整数倍，模型会按 sector 循环搬运并将 used length 设为 `desc1_len+1`。
...
该 queue 子集已经覆盖 feature negotiation、4 条 split queue 的独立配置和 notify、单队列 `QueueReset`、descriptor/avail/used ring 的基本读写路径、indirect descriptor 链、一次 notify 多 pending entry drain、event idx 中断节流、普通 avail flags 中断控制、flush status-only 请求、discard/write-zeroes range 请求和基础设备 reset，但还没有实现 packed queue 和动态配置变更通知。
...
- offset 2：`IIR/FCR`。无 UART 中断 pending 时读 IIR 返回 `0x01`；当 THR empty 且 `IER.THRE=1` 时返回 `0x02`，读出该状态后清除 THRE pending；当 `IER.RX=1` 且 RX FIFO 深度达到 FCR trigger 时返回 `0x04`，表示 received data available；当 RX FIFO 非空但低于 trigger 且持续空闲到 timeout 时返回 `0x0c`，读 RBR 或清 FIFO 后清除；当 `IER.RLS=1` 且发生 overrun/parity/framing/break 任一线状态错误时返回 `0x06`；当 `IER.MS=1` 且 MSR delta sticky 位非零时返回最低优先级 modem status interrupt `0x00`。若 FCR bit0 已启用 FIFO，读 IIR 时额外置 bit7:6=`2'b11`，因此上述低位中断 ID 会表现为 `0xc1/0xc2/0xc4/0xc6/0xcc` 等 16550 兼容返回值。写 FCR 会保存 bit7:6 的 RX trigger，并支持 bit1 清空 RX FIFO。
...
- `vsrc/src/core.sv`
  ...
  - 新增 `mconfigptr` 只读 0、`menvcfg.STCE` WARL 支持、`senvcfg` WARL zero 探测兼容，以及 `stimecmp(0x14d)` S 态访问和 Sstc STIP pending 生成，降低 xv6/Linux 风格启动代码 CSR probe 的直接 illegal 风险，并覆盖新版 xv6 `timerinit()` 路径。
...
- `vsrc/include/csr.sv`
  - 新增 `misa/mvendorid/marchid/mimpid/mconfigptr/menvcfg/senvcfg/stimecmp` 与 `cycle/time/instret/minstret/mcounteren/scounteren` CSR 地址定义，用于标准 CSR 解码和访问门控。
  - 新增 `MENVCFG_STCE_BIT/MENVCFG_MASK/SENVCFG_MASK`，当前只开放 Sstc 所需的 `menvcfg.STCE`。
- `vsrc/util/MMU.sv`
...
- `vsrc/test/s_software_interrupt_tb.sv`
  - 新增独立 core 级 Verilator testbench，验证 `swint` 在 `mideleg.SSIP=1`、`mie.SSIE=1`、`mstatus.SIE=1` 后进入 S software interrupt trap，覆盖 CLINT `msip` 到 core 的 S 级交付路径。
- `vsrc/test/s_external_interrupt_tb.sv`
  - 新增独立 core 级 Verilator testbench，验证 `exint` 在 `mideleg.SEIP=1`、`mie.SEIE=1`、`mstatus.SIE=1` 后进入 S external interrupt trap，覆盖 PLIC 到 core 的 S 级交付路径。
...
- `vsrc/test/mstatus_restrict_tb.sv`
  - 新增独立 core 级 Verilator testbench，分别设置 `mstatus.TVM/TW/TSR` 后从 M 态 `mret` 到 S 态，验证受限 `satp` 访问、`SFENCE.VMA`、`WFI` 和 `SRET` 均以 `mcause=2` 进入 M trap。
- `vsrc/test/mstatus_mprv_tb.sv`
  - 新增独立 core 级 Verilator testbench，验证 M 态清除 `MPRV` 时同一 PMP 区域仍按 M 态访问放行，设置 `MPRV|MPP=U` 后同一 load 按 U 权限产生 load access fault。
- `vsrc/test/csr_counter_tb.sv`
  - 新增独立 core 级 Verilator testbench，验证 M/U 态可读 `cycle/time/instret`、M 态可写 `mcounteren/scounteren`，并验证 U 态写只读 `cycle` 产生 illegal instruction trap。
- `vsrc/test/csr_machine_id_tb.sv`
  - 新增独立 core 级 Verilator testbench，验证 M 态读取 `misa/mvendorid/marchid/mimpid/mhartid` 合法，写 `misa` 被忽略且不 trap，并验证写只读机器 ID CSR 会产生 illegal instruction trap。
- `vsrc/test/csr_envcfg_tb.sv`
  - 新增独立 core 级 Verilator testbench，验证 `mconfigptr` 只读 0，`menvcfg` 只保留 `STCE`，`senvcfg` M/S 态读写保持 WARL zero。
- `vsrc/test/sstc_tb.sv`
  - 新增独立 core 级 Verilator testbench，模拟新版 xv6 `timerinit()` 相关路径：M 态打开 `menvcfg.STCE`，降到 S 态后写 `stimecmp`，并在 `mcycle >= stimecmp` 时以 delegated STIP 进入 S trap。
- `vsrc/test/xv6_start_csr_tb.sv`
  - 新增独立 core 级 Verilator testbench，按 xv6 `start.c` 风格配置 `satp/medeleg/mideleg/sie/pmpaddr0/pmpcfg0/menvcfg/mcounteren/stimecmp/mepc/mstatus`，并验证 `mret` 后进入 S-mode 且 S 态可读取 `stimecmp`。
- `vsrc/test/amo_d_tb.sv`
  - 新增独立 core 级 Verilator testbench，验证 `AMOSWAP.D/AMOADD.D/LR.D/SC.D` 成功路径，以及无 reservation 的 `SC.D` 返回 1 且不写内存。
- `vsrc/test/m_timer_from_s_tb.sv`
  - 新增独立 core 级 Verilator testbench，验证 S 态运行时若 `mie.MTIE=1` 且 `trint=1`，CPU 会进入 M timer interrupt trap，`mcause=interrupt|7` 且 `mepc` 指向 S 态被打断的 PC。
- `vsrc/test/timervec_ssip_tb.sv`
  - 新增独立 core 级 Verilator testbench，模拟 xv6 timervec 交接：S 态收到 M timer interrupt 后进入 M handler，M handler 写 `sip.SSIP` 并 `mret`，随后 CPU 在 S 态进入 software interrupt trap。
- `vsrc/test/difftest_stubs.sv`
  - 为独立 core test 提供空 difftest 模块，避免链接 DPI-C difftest。
- `vsrc/util/SimMemoryWithVirtio.sv`
  - 新增仿真内存包装器，转发普通 RAM/CLINT 请求并处理 `0x10001000` virtio/simple-block MMIO；将 QEMU/xv6 `0x0200...` CLINT 地址映射到课程框架 `0x3800...` 地址；支持 virtio-blk config 读字段、`ConfigGeneration`、包含 `SIZE_MAX/SEG_MAX/BLK_SIZE/FLUSH/DISCARD/WRITE_ZEROES` 的 feature negotiation、`Status=0` reset、4 条 split queue 的独立 descriptor/avail/used ring、按 queue id 处理 `QueueNotify`、`QueueReset` 单队列重置、indirect descriptor、一次 `QueueNotify` drain 多个 pending avail entry、event idx block read/write 子集、`VIRTIO_BLK_T_FLUSH` status-only 请求、`VIRTIO_BLK_T_DISCARD` no-op 成功请求和 `VIRTIO_BLK_T_WRITE_ZEROES` 清零请求，并支持 `+simple_blk_image=...` 或默认 `build/xv6/fs.img` 从二进制镜像初始化 simple-block disk。
  - 修复标准 virtqueue IN/OUT data descriptor 只搬运首个 512B sector 的问题，按 `desc1_len >> 9` 循环搬运多个 sector，支持 xv6 1024B filesystem block，并返回 `desc1_len + 1` 的 used length。
  - 新增 PLIC MMIO 模型，拦截 `0x0c000000` 到 `0x0fffffff`，支持 source priority、pending、M/S enable、threshold、claim/complete。
  - 新增 `0x10000000` 16550 UART MMIO 模型，支持 LCR/DLAB、DLL/DLM、IER、FCR trigger/clear、IIR FIFO-enabled bits、MCR loopback、SCR、LSR overrun/parity/framing/break、break-only line-status、MSR modem-status delta、THR TX 输出、THRE interrupt、RX FIFO timeout、16B RBR RX FIFO 和 RX/THRE/RLS/MS interrupt 到 PLIC source 10。
- `vsrc/SimTop.sv`
  - 将仿真 UART TX/RX 端口接到 difftest 顶层 `io_uart_out_valid/io_uart_out_ch/io_uart_in_*`。
  - 新增 `+uart_input_hex=...` 和 `+uart_input_gap=...` 仿真 UART 输入脚本：在控制台输出匹配 `init: starting sh` 后，把十六进制编码的命令按可配置间隔送入 16550 RX FIFO，用于 xv6 shell 自动交互检查。默认间隔避免 18 字节命令瞬间填满 16B FIFO 时丢字符。
...
- `vsrc/test/uart_mmio_tb.sv`
  - 新增 UART MMIO 定向测试，覆盖 xv6 常用初始化路径、TX 输出、RX ready、RBR FIFO 顺序读取、FCR trigger/clear、IIR FIFO-enabled bits、overrun、parity/framing/break line-status interrupt、break-only 不入队、modem-status loopback interrupt、THRE interrupt、RX FIFO timeout 和 PLIC source 10 claim。
...
- `tools/check_xv6_boot.py`
  ...
  - 新增 marker early-stop：边读取 emu 输出边做 ANSI/计时行过滤，一旦目标字符串出现就优先停止顶层 `make` 的 emu 子进程并判定成功；Linux `/proc` 子进程信息不可用时回退到停止整个进程组。如果需要完整跑到 cycle limit，可加 `--no-early-stop`。
- `ready-to-run/xv6/`
...
```text
amo_d_swap_add [OK]
amo_d_lr_sc_success [OK]
amo_d_sc_fail [OK]
AMO.D directed test passed.
```

该测试直接实例化 `core` 和一个 64 位数据槽，先用 `AMOSWAP.D` 验证返回旧 64 位值并写入新值，再用 `AMOADD.D` 验证 64 位加法写回；随后执行 `LR.D/SC.D` 成功路径，确认 `SC.D` 返回 0 并更新内存，最后在没有 reservation 的情况下再次执行 `SC.D`，确认返回 1 且内存保持不变。
...
### 7.9.1 S-mode software interrupt 定向测试
...
```text
s_software_interrupt [OK]
S-mode software interrupt directed test passed.
```

该测试直接实例化 `core`，手写最小指令流：M 态设置 `mideleg.SSIP=1`、`mie.SSIE=1`、`stvec=0x80`、`mepc=0x40`、`mstatus.MPP=S/SIE=1`，随后执行 `mret`。测试保持 `swint=1`，确认 CPU 返回 S 态后进入 S software interrupt trap，检查 `scause=0x8000000000000001`、`sepc=0x40`，并确认被打断的 S 态普通指令没有先提交。

### 7.9.2 S-mode external interrupt 定向测试
...
```text
s_external_interrupt [OK]
S-mode external interrupt directed test passed.
```

该测试直接实例化 `core`，手写最小指令流：M 态设置 `mideleg.SEIP=1`、`mie.SEIE=1`、`stvec=0x80`、`mepc=0x40`、`mstatus.MPP=S/SIE=1`，随后执行 `mret`。测试保持 `exint=1`，确认 CPU 返回 S 态后进入 S external interrupt trap，检查 `scause=0x8000000000000009`、`sepc=0x40`，并确认被打断的 S 态普通指令没有先提交。

### 7.9.3 M timer from S-mode 定向测试
...
```text
m_timer_from_s_mode [OK]
M-mode timer from S-mode directed test passed.
```

该测试直接实例化 `core`，模拟 xv6 stock timervec 常用路径：M 态设置 `mtvec=0x100`、`mie.MTIE=1`、`mepc=0x40`、`mstatus.MPP=S`，随后执行 `mret` 进入 S 态并保持 `trint=1`。测试确认 CPU 在 S 态运行时不会错误进入 S timer trap，而是按 M timer interrupt 进入 M trap，`mcause=0x8000000000000007`，`mepc=0x40`。这覆盖了低特权级运行时 M 级 timer interrupt 仍可抢占进入 M 态的路径。

### 7.9.4 Timervec SSIP handoff 定向测试
...
```text
timervec_mtimer_to_mmode [OK]
timervec_posts_ssip [OK]
timervec_ssip_to_smode [OK]
Timervec SSIP directed test passed.
```

该测试直接实例化 `core`，模拟 xv6 timervec 中常见的中断交接：M 态初始化 `mideleg.SSIP=1`、`mie.MTIE/SSIE=1`、`mtvec=0x80`、`stvec=0x100`，然后 `mret` 进入 S 态。测试保持 `trint=1`，确认 CPU 先以 M timer interrupt 进入 M handler；handler 写 `sip.SSIP=1` 后 `mret`，随后 CPU 不提交被打断的 S 态普通指令，而是进入 S software interrupt handler，检查 `scause=interrupt|1` 且 `sepc=0x40`。
...
### 7.11.1 mstatus TSR/TW/TVM 定向测试
...
```text
mstatus_tvm_blocks_satp [OK]
mstatus_tvm_blocks_sfence [OK]
mstatus_tw_blocks_wfi [OK]
mstatus_tsr_blocks_sret [OK]
mstatus restrict directed test passed.
```

该测试直接实例化 `core`，每个子项都从 reset 后的 M 态开始，设置 `mtvec=0x100`、`mepc=0x40` 和 `mstatus.MPP=S`，再附加打开 `TVM/TW/TSR` 中对应限制位并执行 `mret` 进入 S 态。测试分别验证 `TVM=1` 时 S 态访问 `satp` 和执行 `SFENCE.VMA` 会以 illegal instruction 进入 M trap，`TW=1` 时 S 态 `WFI` 进入 M trap，`TSR=1` 时 S 态 `SRET` 进入 M trap；四个场景均检查 `mcause=2`、`mepc=0x40`，并确认受限指令后的普通指令未提交。

### 7.11.2 CSR counter 定向测试
...
csr_counter_counteren_writes [OK]
...
```text
csr_machine_id_reads [OK]
csr_misa_read_write_ignored [OK]
csr_machine_id_readonly_write_illegal [OK]
CSR machine-id directed test passed.
```
...
```text
csr_mconfigptr_read_zero [OK]
csr_menvcfg_stce_warl [OK]
csr_senvcfg_smode_access [OK]
CSR envcfg directed test passed.
```

该测试直接实例化 `core`，先在 M 态读取 `mconfigptr/menvcfg/senvcfg`，确认均合法返回 0；随后向 `menvcfg/senvcfg` 写入全 1 并再次读取，确认 `menvcfg` 只保留 `STCE` 位、`senvcfg` 仍按 WARL zero 处理，写入不会触发 trap。最后通过 `mret` 进入 S 态，在 S 态读写 `senvcfg`，确认该 supervisor CSR 不会被错误判为 illegal，并仍保持 0。
...
该测试直接实例化 `SimMemoryWithVirtio`，用 8-bit CBus 访问模拟 xv6 的 UART 初始化流程：先设置 `LCR.DLAB` 写 DLL/DLM，再恢复 8N1、写 IER/MCR/SCR，写 THR 并检查 `uart_out_valid/ch` 输出字符 `A`。随后配置 PLIC source 10，注入 RX 字符 `B`，验证 `LSR.RX_READY`、`IIR=0x04`、PLIC pending/exint、读 RBR 清 RX-ready，以及 PLIC claim/complete。最后覆盖 16B FIFO 顺序读出、FCR trigger=4 时前三个字符不触发中断/第四个字符触发、FCR bit1 清 RX FIFO、启用 FIFO 后 IIR bit7:6 返回 `11`、满 FIFO 反压输入、强制 overrun 后 IIR line-status 优先级、LSR sticky overrun 读后清除；再注入 parity/framing/break，验证 LSR PE/FE/BI/FIFO-error 位、IIR line-status 优先级和 PLIC claim/complete；随后用 `io_uart_in_ch=8'hff` 搭配 break 标记验证 break-only 不入 RBR、LSR 只呈现 BI/FIFO-error 且仍能触发 PLIC claim；打开 `MCR.loopback` 验证 MSR 当前线状态、delta 读后清除、IIR modem status `0xc0` 和 PLIC claim/complete；启用 THRE 后 IIR `0xc2` 和 THR 写入后的再次 THRE interrupt，以及低于 trigger 的 RX FIFO timeout `IIR=0xcc`。
...
virtio_queue1_reset_done [OK]
virtio_queue1_reset_num [OK]
virtio_queue1_reset_ready [OK]
virtio_queue1_reset_desc_low [OK]
virtio_queue3_num_max [OK]
virtio_queue3_num_initial [OK]
virtio_queue4_num_max_zero [OK]
...
在 virtio/disk MMIO 改成 4 条 split queue 后，已重新运行该 smoke 测试，CLINT、PLIC S context、UART、标准 virtqueue read/write/readback、virtio source 和多源 pending 仲裁仍全部通过。

### 7.16.1 xv6 boot 运行入口

运行方式：

```bash
make test-labplus-xv6boot XV6_KERNEL=/path/to/kernel.bin XV6_FS=/path/to/fs.img
```
...
新增 `xv6-prepare-images` 用于准备上述默认路径：

```bash
make xv6-prepare-images XV6_SRC=/path/to/xv6-riscv
```

该目标会调用 `tools/prepare_xv6_images.py`，默认读取 `$(XV6_SRC)/kernel/kernel` 和 `$(XV6_SRC)/fs.img`，用 `riscv64-unknown-elf-objcopy` 或 `riscv64-linux-gnu-objcopy` 生成 `ready-to-run/xv6/kernel.bin`，并复制 `ready-to-run/xv6/fs.img`。也可以通过 `XV6_KERNEL_ELF=/path/to/kernel`、`XV6_FS_IMG=/path/to/fs.img` 和 `XV6_OBJCOPY=/path/to/objcopy` 显式指定输入。当前无输入时验证输出为：
...
### 7.16.1.1 xv6 shell 交互 smoke

在 boot 到 `init: starting sh` 后，进一步验证 shell 是否能接收 UART 输入并执行用户命令。新增目标：

```bash
CCACHE_DISABLE=1 make test-labplus-xv6shell-check
```

该目标通过 `+uart_input_hex=6563686f207876362d7368656c6c2d6f6b0a` 注入 `echo xv6-shell-ok\n`，默认 `+uart_input_gap=100000`，并检查日志中是否出现 `xv6-shell-ok`。关键输出如下：

```text
xv6 kernel is booting
now = 420s
init: starting sh
uart input script armed after trigger: init: starting sh
ech
...
随后对 `tools/check_xv6_boot.py` 增加 marker early-stop，并将 `test-labplus-xv6boot` 改为复用已有 `build/emu` 的增量构建。复测 `XV6_BOOT_EXPECT='xv6 kernel is booting'` 时，第二次运行输出 `Nothing to be done for 'emu'`，总耗时降到约 2.3 秒；复测 `test-labplus-xv6shell-check` 时，`xv6-shell-ok` 出现后立即停止 emu，不再额外等待到 650M cycle 上限。后续又把 early-stop 的终止范围从整个进程组缩小为优先终止 emu 子进程，快速复测 `XV6_BOOT_EXPECT='xv6 kernel is booting' XV6_MAX_CYCLES=100000000` 仍可通过，输出中只保留正常的 make target `Terminated` 收尾，不再出现 `wait: No child processes`。

### 7.16.1.2 xv6 `ls` 目录读取 smoke

在 `echo` 命令确认 shell 交互之后，继续加入目录读取测试：

```bash
CCACHE_DISABLE=1 make test-labplus-xv6ls-check
```

该目标通过 `+uart_input_hex=6c730a` 注入 `ls\n`，复用与 shell smoke 相同的 UART 输入节奏，默认检查目录输出中是否出现 `README`。关键输出如下：

```text
init: starting sh
uart input script armed after trigger: init: starting sh
luart input script finished
s
...
..             1 1 1024
README         2 2 2441
xv6 boot marker observed early: 'README'; stopping emulator
xv6 boot marker found: 'README'
```

这个测试比单纯 `echo` 更进一步：`ls` 是从文件系统镜像中加载并执行的用户程序，运行过程中需要 shell fork/exec、目录 inode 读取、virtio block 从 `fs.img` 返回数据，并把目录项经 UART 打印出来。当前能观察到 `README`，说明 no-RVC smoke 镜像的用户态程序执行和基础文件系统读路径已经跑通。

### 7.16.1.3 xv6 `cat README` 文件读取 smoke

在目录项读取通过后，继续验证普通文件内容读取：

```bash
CCACHE_DISABLE=1 make test-labplus-xv6cat-check
```

该目标通过 `+uart_input_hex=63617420524541444d450a` 注入 `cat README\n`，默认检查 README 正文中的 `xv6 is a re-implementation`。关键输出如下：

```text
init: starting sh
uart input script armed after trigger: init: starting sh
cat
Ruart input script finished
EADME
$ xv6 is a re-implementation of Dennis Ritchie's and Ken Thompson's Unix
xv6 boot marker observed early: 'xv6 is a re-implementation'; stopping emulator
xv6 boot marker found: 'xv6 is a re-implementation'
```

这个测试覆盖了比 `ls` 更完整的文件数据路径：shell 需要执行 `cat` 用户程序，`cat` 通过 `open/read/write` 读取 README 文件内容，virtio block 模型需要持续返回文件数据 block，UART TX 路径需要输出一段真实文件正文。计时输出可能插入串口文本中间，但 `check_xv6_boot.py` 会过滤 ANSI 计时行并在 marker 出现后停止模拟器。

### 7.16.1.4 xv6 `grep xv6 README` 文本扫描 smoke

继续验证带多个参数的用户程序：

```bash
CCACHE_DISABLE=1 make test-labplus-xv6grep-check
```

该目标通过 `+uart_input_hex=677265702078763620524541444d450a` 注入 `grep xv6 README\n`，默认检查输出中是否出现 `xv6 is`。关键输出如下：

```text
init: starting sh
uart input script armed after trigger: init: starting sh
gre
p uart input script finished
xv6 README
$ xv6 is a re-implementation of Dennis Ritchie's and Ken Thompson's Unix
xv6 boot marker observed early: 'xv6 is'; stopping emulator
xv6 boot marker found: 'xv6 is'
```

该检查在 `cat` 的文件读取基础上增加了 shell 对多个 argv 的传递和 `grep` 用户程序内部的文本扫描逻辑。命令回显可能被 UART 输入节奏和计时输出拆开，但最终能打印匹配行，说明当前 smoke 镜像已经能运行更接近真实 xv6 userland 的基础命令组合。

### 7.16.1.5 xv6 `wc README` 文件统计 smoke

继续验证流式读取完整文件并做用户态计数：

```bash
CCACHE_DISABLE=1 make test-labplus-xv6wc-check
```

该目标通过 `+uart_input_hex=776320524541444d450a` 注入 `wc README\n`，默认检查 README 的字节统计输出 `2441 README`。关键输出如下：

```text
init: starting sh
uart input script armed after trigger: init: starting sh
wc
REuart input script finished
ADME
$ 48 336 2441 README
xv6 boot marker observed early: '2441 README'; stopping emulator
xv6 boot marker found: '2441 README'
```
...
### 7.16.1.6 xv6 文件创建/写入/读回 smoke

在只读文件路径之外，继续验证文件系统写入：

```bash
CCACHE_DISABLE=1 make test-labplus-xv6fswrite-check
```

该目标通过 `+uart_input_hex=6563686f20616263203e20746d700a776320746d700a` 注入两条命令：
...
wc tmp
$ 1 1 4 tmp
xv6 boot marker observed early: '4 tmp'; stopping emulator
xv6 boot marker found: '4 tmp'
```

这个测试会通过 shell redirection 创建 `tmp`，经 `echo` 写入 `abc\n`，再由 `wc` 读回统计。它覆盖了 xv6 文件创建、inode 更新、数据写入、virtio block 写路径和后续读回，比前面的 README 只读 smoke 更接近真实文件系统使用场景。

### 7.16.1.7 xv6 userland 汇总入口
...
该目标顺序运行 `test-labplus-xv6shell-check`、`test-labplus-xv6ls-check`、`test-labplus-xv6cat-check`、`test-labplus-xv6grep-check`、`test-labplus-xv6wc-check` 和 `test-labplus-xv6fswrite-check`。本次已经分别验证六个子目标通过，因此汇总入口主要用于后续改动后的集中回归。
...
```bash
CCACHE_DISABLE=1 make test-labplus-sstc
```

关键输出：

```text
sstc_menvcfg_stce [OK]
sstc_smode_stimecmp_write [OK]
sstc_supervisor_timer_interrupt [OK]
Sstc directed test passed.
```

该测试补充新版 xv6-riscv `timerinit()` 会用到的 Sstc 路径。测试程序先在 M 态写 `menvcfg` 并确认只保留 `STCE` 位，再设置 `mideleg/mie/stvec/mstatus/mepc` 后 `mret` 到 S 态；S 态执行 `csrw stimecmp, x0` 后，CPU 由 `mcycle >= stimecmp` 产生 STIP pending，并在 delegated supervisor timer interrupt 中写入 `scause=interrupt|5`、`sepc` 和 S trap 入口。回归同时重跑了 `test-labplus-csr-envcfg`、`test-labplus-sinterrupt`、`test-labplus-mtimer` 和 `test-labplus-timervec`，确认新路径没有破坏原有 envcfg、S 中断、M timer from S-mode 和 timervec SSIP handoff。

### 7.16.3 xv6 start CSR 定向测试

运行：

```bash
CCACHE_DISABLE=1 make test-labplus-xv6start
```

关键输出：

```text
xv6_start_satp_bare [OK]
xv6_start_delegation [OK]
xv6_start_pmp_all_memory [OK]
xv6_start_sstc_counter_setup [OK]
xv6 start CSR directed test passed.
```

该测试把新版 xv6 `start.c` 早期 M 态初始化序列压成一个 core 级 directed test：关闭分页写 `satp=0`，写入全 1 的 `medeleg/mideleg` 并检查 CPU 按 mask 保留可委托位，写 `sie` 打开 SSIE/STIE/SEIE，按 xv6 写法设置 `pmpaddr0=0x003f_ffff_ffff_ffff` 和 `pmpcfg0=0x0f` 允许后续 S 态访问内存，写 `menvcfg` 只保留 `STCE`，写 `mcounteren.TM`，用 `time+2047` 初始化 `stimecmp`，最后设置 `mepc=0x40`、`mstatus.MPP=S` 并执行 `mret`。S 态入口再读取一次 `stimecmp`，确认 `STCE` 打开后 supervisor 访问该 CSR 不会 illegal。
...
该入口顺序运行 `test-labplus-vivado-precheck`、`test-labplus-board-device`、`test-labplus-board-soc-trace`、`test-labplus-pagefault`、`test-labplus-sinterrupt`、`test-labplus-ssoftint`、`test-labplus-sextint`、`test-labplus-mtimer`、`test-labplus-timervec`、`test-labplus-sstc`、`test-labplus-xv6start`、`test-labplus-sfence`、`test-labplus-wfi`、`test-labplus-mstatus-restrict`、`test-labplus-counters`、`test-labplus-mcountinhibit`、`test-labplus-csr-id`、`test-labplus-csr-envcfg`、`test-labplus-amo-d`、`test-labplus-clint`、`test-labplus-plic`、`test-labplus-uart`、`test-labplus-virtio` 和 `test-labplus-xv6smoke`。本次回归全部通过，关键收尾输出包括：
...
这个集合不替代 Vivado 综合/实现和真实上板串口输出，但可以在没有板子的情况下快速确认 xv6/pre-board 相关的 Vivado 工程配置、已有 bitstream/report 健康状态、CPU 特权、counter/mcountinhibit/机器 ID/ISA/envcfg/Sstc CSR、MMU fault、S/M 态中断和 timervec SSIP 交接、SFENCE/WFI/mstatus TSR/TW/TVM 限制、CLINT/PLIC/UART/virtio 仿真模型没有被后续改动破坏。
...
本次 Lab+ 新增完成了 atomic extension、AMO.D、8-entry PMP/privfull 支持、`EBREAK` 断点异常、`FENCE/FENCE.I`/`WFI` 兼容，并加入顺序取指提前、CBus fast path、8B 指令行缓冲和 32 项 2-bit BHT 动态分支预测等前端性能优化。继续推进 xv6 主 Track 时，已完成 S-mode、`SRET`、trap delegation、S 态中断 pending 委托转换、S 态软件/外部中断交付、S 态运行时 M timer trap 覆盖、timervec SSIP handoff 覆盖、Sstc `menvcfg.STCE/stimecmp` 兼容、xv6 start CSR 序列、标准 counter CSR 与 `mcounteren/scounteren/mcountinhibit` 门控、机器 ID/ISA/envcfg CSR 兼容、MMU page fault/PTE 基础权限检查、`SUM/MXR` 权限补充、PTE A/D 位硬件更新、`SFENCE.VMA` 合法/非法路径覆盖、S/M 态 `WFI` 合法 no-op、mstatus TSR/TW/TVM 限制、CLINT legacy/QEMU 地址兼容、可从镜像初始化且支持 virtio-blk config、基础 feature negotiation、4 条 split queue virtqueue/QueueReset/indirect descriptor/multi-pending notify/event idx/reset/flush/discard/write-zeroes 子集的 virtio/disk MMIO、仿真侧 PLIC MMIO 模型、最小 16550 UART TX/RX FIFO/THRE/timeout/IIR FIFO-enabled bits/overrun/parity/framing/break/break-only/modem-status 模型，以及 `test-labplus-xv6boot` raw kernel/fs.img 运行入口。本轮进一步修复 virtio 多扇区 data descriptor，加入 no-RVC xv6 boot smoke 镜像，并恢复 virtio disk 与 UART TX 的中断驱动 sleep/wakeup 路径，通过 `test-labplus-xv6boot-check` 跑到 `init: starting sh`，通过 `test-labplus-xv6shell-check` 注入并执行 `echo xv6-shell-ok`，通过 `test-labplus-xv6ls-check` 执行 `ls` 并观察到 `README` 目录项，通过 `test-labplus-xv6cat-check` 执行 `cat README` 并观察到 README 正文，通过 `test-labplus-xv6grep-check` 执行 `grep xv6 README` 并观察到匹配行，通过 `test-labplus-xv6wc-check` 执行 `wc README` 并观察到字节统计，再通过 `test-labplus-xv6fswrite-check` 执行 `echo abc > tmp; wc tmp` 并观察到写入后统计。上板调试中先后修复了 UART 自动字符串发送时 `txData` 中途覆盖、`Hello World` 大小写不一致、TX_DATA backpressure 隔字丢失，以及 BRAM read response stale-ready 导致第二行 `R` 后取指错误的问题。AMO 实现利用现有单发访存结构，将 AMO 指令拆成不可被其他指令插入的读-改-写序列，并为 `LR.W/SC.W/LR.D/SC.D` 添加 reservation 状态。性能优化将 Lab1 extra 周期数从 185976 降到 93020，将 Lab4 周期数从 208529 降到 110572，atomicity 从 372 降到 197。最终 atomicity、Lab1 extra 和 Lab4 已完成动态 BHT 增量快速回归；Lab+ privileged sys-test、S-mode interrupt directed test、S software/external interrupt directed test、M timer from S-mode directed test、timervec SSIP directed test、Sstc directed test、xv6 start CSR directed test、SFENCE.VMA directed test、WFI directed test、mstatus restrict directed test、CSR counter/mcountinhibit/machine-id/envcfg directed test、AMO.D directed test、MMU page fault、CLINT/PLIC/UART/virtio/xv6 smoke、xv6 boot 到 shell banner、shell 命令交互、`ls` 目录读取、`cat README` 文件读取、`grep xv6 README` 文本扫描、`wc README` 文件统计与文件写入读回检查、Vivado pre-board、Nexys4 board device、`soc_top` board trace、Lab5、Lab6 沿用前序回归结果。
