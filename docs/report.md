...
- 新增仿真侧 16550 UART MMIO 模型：在 `0x10000000` 兼容 xv6/QEMU UART 初始化、TX 输出、16B RX FIFO/RBR 读取，并将 UART RX interrupt 接到 PLIC source 10。
...
- offset 2：`IIR/FCR`。无 UART 中断 pending 时读 IIR 返回 `0x01`；当 `IER.RX=1` 且 RX 缓冲非空时返回 `0x04`，表示 received data available。
- offset 3/4/7：`LCR/MCR/SCR` 可读写。
- offset 5：`LSR` 固定保持 `THR empty/transmitter empty=1`，并根据 RX FIFO 非空状态动态维护 bit0 `RX_READY`，满足 xv6 `uartputc` 和 `uartgetc` 的轮询条件。

`SimTop` 不再把 difftest UART 端口固定为 0，而是将 `SimMemoryWithVirtio` 的 TX 输出连接到 `io_uart_out_valid/io_uart_out_ch`。RX 侧将 `io_uart_in_ch=8'hff` 视为无字符，其它值会进入 16 字节 RX FIFO；若 `IER.RX=1`，同时置位 PLIC source 10 pending，软件可以通过 PLIC claim/complete 接收 UART RX 外部中断。FIFO 满时 `io_uart_in_valid=0`，向 host 侧反压输入。
...
- `vsrc/util/SimMemoryWithVirtio.sv`
  ...
  - 新增 `0x10000000` 16550 UART MMIO 模型，支持 LCR/DLAB、DLL/DLM、IER、MCR、SCR、LSR、THR TX 输出、16B RBR RX FIFO 和 RX interrupt 到 PLIC source 10。
- `vsrc/SimTop.sv`
...
- `vsrc/test/uart_mmio_tb.sv`
  - 新增 UART MMIO 定向测试，覆盖 xv6 常用初始化路径、TX 输出、RX ready、RBR FIFO 顺序读取和 PLIC source 10 claim。
...
该测试直接实例化 `SimMemoryWithVirtio`，用 8-bit CBus 访问模拟 xv6 的 UART 初始化流程：先设置 `LCR.DLAB` 写 DLL/DLM，再恢复 8N1、写 IER/MCR/SCR，写 THR 并检查 `uart_out_valid/ch` 输出字符 `A`。随后配置 PLIC source 10，注入 RX 字符 `B`，验证 `LSR.RX_READY`、`IIR=0x04`、PLIC pending/exint、读 RBR 清 RX-ready，以及 PLIC claim/complete。最后连续注入 `C/D` 两个字符，确认 RBR 按 FIFO 顺序读出，读出 `C` 后仍保持 ready/pending，读出 `D` 后清空。
...
- 更完整的 16550 FIFO trigger、overrun 和 LSR 错误位。
