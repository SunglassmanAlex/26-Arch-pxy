...
该 queue 子集已经覆盖 feature negotiation、2 条 split queue 的独立配置和 notify、单队列 `QueueReset`、descriptor/avail/used ring 的基本读写路径、indirect descriptor 链、一次 notify 多 pending entry drain、event idx 中断节流、普通 avail flags 中断控制、flush status-only 请求、discard/write-zeroes range 请求和基础设备 reset，但还没有实现 packed queue 和动态配置变更通知。
