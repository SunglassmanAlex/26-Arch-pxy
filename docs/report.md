...
- 新增可直接复测的 no-RVC xv6 boot smoke 镜像：使用 `rv64ima_zicsr_zifencei/lp64` 构建并放入 `ready-to-run/xv6/`，当前 boot check 已观察到 `init: starting sh`，shell 交互检查已观察到 `xv6-shell-ok`，`ls` 检查已观察到 `README` 目录项，`cat README` 检查已观察到 README 正文。
