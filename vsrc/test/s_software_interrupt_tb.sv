...
module s_software_interrupt_tb
    import common::*;
    import csr_pkg::*;
;
    ...
    localparam word_t S_SOFTWARE_CAUSE = 64'h8000_0000_0000_0001;
    ...
    function automatic u32 instr_at(input addr_t addr);
        unique case (addr)
            ...
            PCINIT + 64'h08: instr_at = addi(5'd1, 5'd0, 12'sh002);
    ...
    initial begin
        ...
        for (int cycle = 0; cycle < 140; cycle += 1) begin
            ...
            if (dut.gpr[5] == 64'd1) begin
                ...
                if (dut.csr_scause != S_SOFTWARE_CAUSE) begin
                ...
                $display("s_software_interrupt [OK]");
                $display("S-mode software interrupt directed test passed.");
                $finish;
            end
            if (dut.csr_mcause == S_SOFTWARE_CAUSE) begin
                $fatal(1, "software interrupt was incorrectly handled as an M trap");
            end
        end

        $fatal(1, "timed out waiting for delegated S software interrupt");
    end
